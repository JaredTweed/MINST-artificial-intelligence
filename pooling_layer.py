...
def pooling_layer_forward(input, layer):
    ...
    # print('POOLING START')
    # print(input['data'].shape) # prints (108, 2)
    input_image = np.reshape(input['data'], (h_in, w_in, c, batch_size), order='F')
    # print(inputImage.shape)
    padded_input = np.pad(input_image, [(pad, pad), (pad, pad),(0,0),(0,0)], mode='constant')
    # print(h_in)
    # print(w_in)
    # print(inputImage)
    # print(k)
    ...
    for h in range(h_out):
        for w in range(w_out):
            kernel_matrix = padded_input[h*stride : h*stride + k, w*stride : w*stride + k, :, :]
            # print(kernelMatrix.shape)
            pooled_value = np.max(kernel_matrix, axis=(0, 1)) # computes the max of the 0 and 1
            output['data'][h, w, :, :] = pooled_value
